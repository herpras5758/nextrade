    ...
    async (req, reply) => {
      ...
      return withTenant(tenantId, async (client) => {
        ...
        const { rows: files } = await client.query(
          `SELECT id, original_filename, detected_type, detected_category,
                  action, confidence_tier, match_confidence,
                  matched_shipment_id, is_duplicate_of, user_override, analysis_detail,
                  s.shipment_number
